...
export default class TsSectionAnalyzer extends TsBase {
    ...
    _programNumbers: number[];

    _originalNetworkId: number;
    _transportStreamId: number;
    _serviceIds: number[];

    _network: Network;
    _services: { [sid: number]: Service };
    ...
    onPat(tsSection) {
        ...
        if (tsSection.getCurrentNextIndicator() === 0) {
            return;
        }
    ...
    onSdt(tsSection) {
        ...
        for (let i = 0, l = objSection.services.length; i < l; i++) {
            ...
            const _service: Service = TsUtil.getNestedObject(this._services, [service.service_id]);
    ...
    hasTime() {
        return this._tdtFlag || this._totFlag;
    }
