...
class TsCommonDataManager extends TsBase {
    ...
    onDsmcc(tsSection) {
        ...
        switch (tableId) {
            case 0x3B: {
                ...
                // Check download id
                if (!TsUtil.checkNestedObject(this.downloadIds, [objMessage.downloadId])) break;

                // Get download id
                const downloadId = TsUtil.getNestedObject(this.downloadIds, [objMessage.downloadId]);
                    ...
                    for (let j = 0, l2 = tsCarouselDescriptors.length; j < l2; j++) {
                        ...
                        switch (tsCarouselDescriptor.getDescriptorTag()) {
                            case 0x02: {
                                ...
                                module.moduleName = new TsChar(objDescriptor.text);
            ...
            case 0x3C: {
                ...
                // Check download id
                if (!TsUtil.checkNestedObject(this.downloadIds, [objMessage.downloadId])) return;
                ...
                if (downloadId.completeModules.length === downloadId.numberOfModules) {
                        ...
                        switch (_module.name) {
                            ...
                            case "CS_FEATURE": {
                                ...
                                for (let j = 0, l2 = downloadId.compatibilities.length; j < l2; j++) {
                                    ...
                                    switch (compatibility.model) {
                                        case 0xFFFA: {
                                            this.features.terrestrial = feature;
                            ...
                            case "CS_KEYWORD": {
                                ...
                                for (let j = 0, l2 = downloadId.compatibilities.length; j < l2; j++) {
                                    ...
                                    switch (compatibility.model) {
                                        case 0xFFFA: {
                                            this.keywords.terrestrial = keyword;
                            ...
                            case "CS_LOGO-05": {
                                ...
                                for (let j = 0, l2 = objDataModule.logos.length; j < l2; j++) {
                                    ...
                                    for (let k = 0, l3 = moduleLogo.services.length; k < l3; k++) {
                                        ...
                                        const logoRef = TsUtil.getNestedObject(this.logoRefs, [service.original_network_id, service.transport_stream_id, service.service_id]);
                                    ...
                                    for (let k = 0, l3 = originalNetworkIds.length; k < l3; k++) {
                                        ...
                                        const logo = TsUtil.getNestedObject(this.logos, [originalNetworkId, moduleLogo.logo_id]);
                    ...
                    TsUtil.removeNestedObject(this.downloadIds, [objMessage.downloadId]);
                }
                ...
                switch (tsDescriptor.getDescriptorTag()) {
                    case 0xCF: {
                        ...
                        switch (objDescriptor.logo_transmission_type) {
                            case 1: {
                                const logoRef = TsUtil.getNestedObject(this.logoRefs, [objSection.original_network_id, objSection.transport_stream_id, service.service_id]);

                                logoRef.logoId = objDescriptor.logo_id;

                                TsUtil.getNestedObject(this.downloadDataIds, [objSection.original_network_id, objDescriptor.download_data_id]);
                            ...
                            case 3: {
                                const simpleLogo = TsUtil.getNestedObject(this.simpleLogos, [objSection.original_network_id, objSection.transport_stream_id, service.service_id]);
            ...
            for (let j = 0, l2 = tsDescriptors.length; j < l2; j++) {
                ...
                switch (tsDescriptor.getDescriptorTag()) {
                    case 0xC9: {
                        ...
                        TsUtil.getNestedObject(this.downloadIds, [objDescriptor.download_id]);
                    }
                }
            }
        }
    ...
    onCdt(tsSection) {
        ...
        const tableId = tsSection.getTableId();
        const subTable = TsUtil.getNestedObject(this.versions, [tableId, tsSection.getOriginalNetworkId(), tsSection.getDownloadDataId()]);
        ...
        if (!TsUtil.checkNestedObject(this.downloadDataIds, [tsSection.getOriginalNetworkId(), tsSection.getDownloadDataId()])) return;
        ...
        switch (objSection.data_type) {
            case 0x01: {
                ...
                const logo = TsUtil.getNestedObject(this.logos, [objSection.original_network_id, objDataModule.logo_id]);
        ...
        if (TsUtil.checkSections(subTable)) {
            TsUtil.removeNestedObject(this.downloadDataIds, [objSection.original_network_id, objSection.download_data_id]);
        }
    }

    hasGenre(type) {
        return this.genres.hasOwnProperty(type);
    }

    hasFeature(type) {
        return this.features.hasOwnProperty(type);
    }

    hasKeyword(type) {
        return this.keywords.hasOwnProperty(type);
    }

    hasLogos(onid, logoId) {
        if (!TsUtil.checkNestedObject(this.logos, [onid, logoId])) return false;

        const logo = TsUtil.getNestedObject(this.logos, [onid, logoId]);
    ...
    hasSimpleLogo(onid, tsid, sid) {
        return TsUtil.checkNestedObject(this.simpleLogos, [onid, tsid, sid]);
    }
    ...
    hasLogos(onid, logoId) {
        if (!TsUtil.checkNestedObject(this.logos, [onid, logoId])) return null;
    ...
    hasSimpleLogo(onid, tsid, sid) {
        if (TsUtil.checkNestedObject(this.simpleLogos, [onid, tsid, sid])) return null;

        const simpleLogo = TsUtil.getNestedObject(this.simpleLogos, [onid, tsid, sid]);
