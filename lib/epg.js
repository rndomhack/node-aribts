    ...
    addEit(pid, objEit) {
        ...
        if (!this.epg[onid][tsid].hasOwnProperty(sid)) {
            ...
            for (let i = 0; i < 0x08; i++) {
                this.epg[onid][tsid][sid].basic_flags.flags.push({
                    flag: new Buffer(16).fill(0x00),
                    ignore: new Buffer(16).fill(0xFF),
                    version_number: -1
                });

                this.epg[onid][tsid][sid].extended_flags.flags.push({
                    flag: new Buffer(16).fill(0x00),
                    ignore: new Buffer(16).fill(0xFF),
                    version_number: -1
                });
        ...
        if (objEit.table_id !== 0x4E && objEit.table_id !== 0x4F) {
            ...
            let table_id = objEit.table_id & 0x0F;
            let last_table_id = objEit.last_table_id & 0x0F;
            ...
            let target_flags = table_id < 0x08 ? objService.basic_flags : objService.extended_flags;

            if ((target_flags.last_table_id !== last_table_id) ||
            ...
            for (let i = last_segment_number + 1; i < 0x10 ; i++) {
            ...
            target_flags.last_table_id = last_table_id;
        ...
        return Object.keys(this.epg).every(onid => {
            return Object.keys(this.epg[onid]).every(tsid => {
                return Object.keys(this.epg[onid][tsid]).every(sid => {
                    ...
                    return [objService.basic_flags, objService.extended_flags].every(target_flags => {
                        return target_flags.flags.every(table => {
                            return table.flag.every((segment, i) => segment | table.ignore[i] === 0xFF);
                        });
                    });
                });
            });
