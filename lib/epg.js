...
class TsEpg {
    ...
    addEit(pid, objEit) {
        ...
        objEit.events.forEach(event => {
                    ...
                    case 0x4E:
                        ...
                        descriptor.items.forEach(item => {
                            if (item.item_description_length) {
                                extended_event.item_description = new TsChar(item.item_description_char).decode();

                                if (!extended_event.descriptor.items.hasOwnProperty(extended_event.item_description)) {
                                    extended_event.descriptor.items[extended_event.item_description] = [];
                                }
                            }

                            extended_event.descriptor.items[extended_event.item_description].push(item.item_char);
                        });
            ...
            if (descriptors.extended_event.descriptor !== null) {
                let extended_event = descriptors.extended_event;

                Object.keys(extended_event.descriptor.items).forEach(key => {
                    extended_event.descriptor.items[key] = new TsChar(Buffer.concat(extended_event.descriptor.items[key])).decode();
                });
            }
