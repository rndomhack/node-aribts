...
class TsStream extends stream.Transform {
    ...
    parsePat(pid, objPat) {
        ...
        var detectPmtPids = [];
        var pmtPids = this.options.transPmtPids.slice();
        ...
        this.trans.pat.programs.forEach(program => {
            if (program.program_number === 0) return;

            detectPmtPids.push(program.program_map_PID);
        });

        this.options.transPmtIds.forEach(id => {
            if (id === -1) {
                // Add All Pmt
                pmtPids = detectPmtPids.slice();
            } else {
                // Add Selected Pmt
                if (detectPmtPids.length < id + 1) return;
                if (pmtPids.indexOf(detectPmtPids[id]) !== -1) return;

                pmtPids.push(detectPmtPids[id]);
            }
        });
    ...
    updatePids() {
        var pids = this.options.transPids.slice();

        // Add PAT PID
        pids.push(0x0000);

        if (this.trans.nitPid !== -1) {
            pids.push(this.trans.nitPid);
        }

        if (this.trans.pat !== null) {
            this.trans.pat.programs.forEach(program => {
                if (program.program_number !== 0) return;
                if (pids.indexOf(program.network_PID) !== -1) return;
        ...
        if (this.trans.cat !== null) {
            this.trans.cat.descriptors.forEach(descriptor => {
                if (descriptor.descriptor_tag !== 0x09) return;
                if (pids.indexOf(descriptor.CA_PID) !== -1) return;
        ...
        this.trans.pmtPids.forEach(pmtPid => {
            if (!this.trans.pmt.hasOwnProperty(pmtPid)) return;
            ...
            this.trans.pmt[pmtPid].program_info.forEach(descriptor => {
                if (descriptor.descriptor_tag !== 0x09) return;
                if (pids.indexOf(descriptor.CA_PID) !== -1) return;
            ...
            this.trans.pmt[pmtPid].streams.forEach(_stream => {
                if (pids.indexOf(_stream.elementary_PID) !== -1) return;
    ...
    rebuildPat() {
        ...
        let bufferPat = new tsTable.TsTablePat(new Buffer(0x400).fill(0xFF)).encode(objPat);

        if (bufferPat.length > 183) {
            throw new RangeError("PAT is too long");
        }

        objPacket.data_byte = Buffer.concat([new Buffer(1).fill(0x00), bufferPat]);
        this.trans.rebuild.pat = new TsPacket(new Buffer(188).fill(0xFF)).encode(objPacket);

        this.trans.rebuild.patVersion = (this.trans.rebuild.patVersion + 1) & 0x1F;
    }

    createPat() {
        var bufferPacket = new Buffer(this.trans.rebuild.pat);
