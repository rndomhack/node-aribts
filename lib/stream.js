...
class TsStream extends stream.Transform {
    constructor(options) {
        ...
        this.parser = {
            ...
            pat: new table.TsPatParser(),
            cat: new table.TsCatParser(),
            pmt: new table.TsPmtParser(),
            nit: new table.TsNitParser(),
            sdt: new table.TsSdtParser(),
            bat: new table.TsBatParser(),
            eit: new table.TsEitParser(),
            tdt: new table.TsTdtParser(),
            tot: new table.TsTotParser()
        };
                ...
                sections.forEach(section => {
                    ...
                    if (tableId === 0x00) {
                        ...
                        if (this.listenerCount("eit")) {
                            this.emit("eit", objBasic.PID, this.parser.eit.parse(section));
                        }
                    } else if (tableId === 0x70) {
                        // TDT
                        if (this.listenerCount("tdt")) {
                            this.emit("tdt", objBasic.PID, this.parser.tdt.parse(section));
                        }
                    } else if (tableId === 0x73) {
                        // TOT
                        if (this.listenerCount("tot")) {
                            this.emit("tot", objBasic.PID, this.parser.tot.parse(section));
                        }
                    }
                });
            }
