...
class TsEventManager extends TsBase {
    ...
    onEit(tsSection) {
        ...
        const subTable = TsUtil.getNestedObject(this._versions, [tableId, onid, tsid, sid]);
        ...
        const service = TsUtil.getNestedObject(this._services, [onid, tsid, sid]);
        ...
        if (objSection.table_id !== 0x4E && objSection.table_id !== 0x4F) {
            ...
            if (this._time !== null) {
                // Update past segment
                const currentSegmentOffset = (this._time.getTime() - this._time.getTimezoneOffset() * 60 * 1000) / (3 * 60 * 60 * 1000) & 0x07;
    ...
    hasPresent(onid, tsid, sid) {
        if (!TsUtil.checkNestedObject(this._services, [onid, tsid, sid])) return false;
    ...
    hasSchedules() {
        for (let keys = Object.keys(this._services), i = 0, l = keys.length; i < l; i++) {
            const key = keys[i];
            const originalNetwork = this._services[key];
    ...
    getPresent(onid, tsid, sid) {
        if (!TsUtil.checkNestedObject(this._services, [onid, tsid, sid])) return null;
    ...
    getScheduleAmount(onid, tsid, sid) {
        ...
        if (!TsUtil.checkNestedObject(this._services, [onid, tsid, sid])) return [stored, overall];
