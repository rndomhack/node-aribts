...
class TsEventManager extends TsBase {
    ...
    onEit(tsSection) {
        ...
        const service = TsUtil.getNestedObject(this._events, [onid, tsid, sid]);
    ...
    hasPresent(onid, tsid, sid) {
        if (!TsUtil.checkNestedObject(this._events, [onid, tsid, sid])) return false;
    ...
    hasSchedules() {
        for (let keys = Object.keys(this._events), i = 0, l = keys.length; i < l; i++) {
            const key = keys[i];
            const originalNetwork = this._events[key];
    ...
    getPresent(onid, tsid, sid) {
        if (!TsUtil.checkNestedObject(this._events, [onid, tsid, sid])) return null;
    ...
    getScheduleAmount(onid, tsid, sid) {
        ...
        if (!TsUtil.checkNestedObject(this._events, [onid, tsid, sid])) return [stored, overall];
