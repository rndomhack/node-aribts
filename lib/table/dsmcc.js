...
class TsMessageDownloadInfoIndication {
    constructor(buffer) {
        this.buffer = buffer;
    }
    ...
    decode() {
        ...
        objDii.compatibilityDescriptor = new TsDescriptorCompatibility(reader.readBytes(2 + descriptorLength)).decode();

        objDii.numberOfModules = reader.uimsbf(16);

        for (let i = 0; i < objDii.numberOfModules; i++) {
            objDii.moduleId = reader.uimsbf(16);
            objDii.moduleSize = reader.uimsbf(32);
            objDii.moduleVersion = reader.uimsbf(8);
            objDii.moduleInfoLength = reader.uimsbf(8);
            objDii.moduleInfoByte = reader.readBytes(objDii.moduleInfoLength);
        }

        objDii.privateDataLength = reader.uimsbf(16);
        objDii.privateDataByte = reader.readBytes(objDii.privateDataLength);

        return objDii;
    }
}

class TsMessageDownloadDataBlock {
    constructor(buffer) {
        this.buffer = buffer;
    }
...
class TsTableDsmcc {
    ...
    decode() {
        ...
        if (objDsmcc.table_id === 0x3B) {
            objDsmcc.userNetworkMessage = new TsMessageDownloadInfoIndication(buffer).decode();
        } else if (objDsmcc.table_id === 0x3C) {
            objDsmcc.downloadDataMessage = new TsMessageDownloadDataBlock(buffer).decode();
