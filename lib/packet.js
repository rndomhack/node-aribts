...
class TsPacket {
    ...
    decodeAdaptationField() {
        var buffer = TsPacket.getAdaptationField(this.buffer);
        ...
        if (objAF.adaptation_field_length > 0) {
            ...
            if (objAF.PCR_flag === 1) {
                objAF.program_clock_reference_base = reader.uimsbf(2) * 0x80000000 + reader.uimsbf(31);
                reader.next(6);    // reserved
                objAF.program_clock_reference_extension = reader.uimsbf(9);
            }
            if (objAF.OPCR_flag === 1) {
                objAF.original_program_clock_reference_base = reader.uimsbf(2) * 0x80000000 + reader.uimsbf(31);
                reader.next(6);    // reserved
    ...
    encode(objPacket) {
        var writer = new TsWriter(this.buffer);
