...
class TsDescriptorDownloadContent {
    ...
    decode() {
        ...
        if (objDescriptor.compatibility_flag === 1) {
            let descriptor_length = (reader.buffer[reader.position >> 3] << 8) | reader.buffer[(reader.position >> 3) + 1];
            objDescriptor.compatibility_descriptor = new TsDescriptorCompatibility(reader.readBytes(2 + descriptor_length)).decode();
        }
