...
class TsDescriptorCarouselCompatibleComposite {
    ...
    decode() {
        ...
        while (reader.position >> 3 < 2 + objDescriptor.descriptor_length) {
            let descriptor;
            let descriptor_tag = reader.buffer[reader.position >> 3] << 8;
            let descriptor_length = reader.buffer[(reader.position >> 3) + 1] << 8;
            let buffer = reader.readBytes(2 + descriptor_length);
